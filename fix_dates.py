...
# Create a list of dates from September 2021 to August 2023
def generate_dates(num_dates=473):
    start_date = datetime(2021, 9, 1)
    end_date = datetime(2023, 8, 31)
    
    # Calculate time span and interval
    time_span = (end_date - start_date).total_seconds()
    interval = time_span / num_dates
    
    dates = []
    for i in range(num_dates):
        # Add some randomness to the interval
        random_factor = random.uniform(0.5, 1.5)
        current_date = start_date + timedelta(seconds=i * interval * random_factor)
...
def create_commit_history(num_commits=473):
    # Generate dates
    dates = generate_dates(num_commits)
...
if __name__ == "__main__":
    print("Starting to create commit history with correct dates...")
